...
public class CocoaUIEnhancer implements Runnable {
	...
	private void init()
		...
		invokeMethod(OS.class, "class_addIvar",
				new Object[] { wrapPointer(cls), SWT_OBJECT, wrapPointer(size), new Byte((byte) align), types });

		// Add the action callback
		invokeMethod(OS.class, "class_addMethod",
				new Object[] { wrapPointer(cls), wrapPointer(sel_preferencesMenuItemSelected_), wrapPointer(proc3), "@:@" }); //$NON-NLS-1$
		invokeMethod(OS.class, "class_addMethod",
				new Object[] { wrapPointer(cls), wrapPointer(sel_aboutMenuItemSelected_), wrapPointer(proc3), "@:@" }); //$NON-NLS-1$

		invokeMethod(OS.class, "objc_registerClassPair", new Object[] { wrapPointer(cls) });
	}
	...
	private final ActionRunnable preferencesAction;
	private final ActionRunnable aboutAction;

	/**
	 * Default constructor
	 */
	public CocoaUIEnhancer(String applicationName, ActionRunnable about, ActionRunnable preferences) {
	...
	private long registerName(String name)
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				try {
					...
					invokeMethod(OS.class, "object_setInstanceVariable", new Object[] { idValue, SWT_OBJECT, wrapPointer(delegateJniRef) });
	...
	private void hookApplicationMenu() {
		try {
			...
			NSMenuItem aboutMenuItem = (NSMenuItem) invokeMethod(NSMenu.class, appMenu, "itemAtIndex",
			...
			if (fHideActionName != null) {
				NSMenuItem hideMenuItem = (NSMenuItem) invokeMethod(NSMenu.class, appMenu, "itemAtIndex",
			...
			if (fQuitActionName != null) {
				NSMenuItem quitMenuItem = (NSMenuItem) invokeMethod(NSMenu.class, appMenu, "itemAtIndex",
						new Object[] { wrapPointer(kQuitMenuItem) });
				quitMenuItem.setTitle(NSString.stringWith(fQuitActionName));
			}

			// enable pref menu
			NSMenuItem prefMenuItem = (NSMenuItem) invokeMethod(NSMenu.class, appMenu, "itemAtIndex",
					new Object[] { wrapPointer(kPreferencesMenuItem) });
			prefMenuItem.setEnabled(preferencesAction != null);

			// disable services menu
			NSMenuItem servicesMenuItem = (NSMenuItem) invokeMethod(NSMenu.class, appMenu, "itemAtIndex",
	...
	void preferencesMenuItemSelected() {
		if (preferencesAction.getVisibility() == ActionRunnable.VISIBILITY_ENABLE) {
			preferencesAction.run(ActionMonitor.empty);
		}
	}

	void aboutMenuItemSelected() {
		if (aboutAction.getVisibility() == ActionRunnable.VISIBILITY_ENABLE) {
	...
	static long actionProc(long id, long sel, long arg0) throws Exception {
		...
		CocoaUIEnhancer delegate = (CocoaUIEnhancer) invokeMethod(OS.class, "JNIGetObject", new Object[] { wrapPointer(jniRef[0]) });
	...
	private static long[] OS_object_getInstanceVariable(long delegateId, byte[] name)
		...
		if (PTR_CLASS == long.class) {
			method = clazz.getMethod("object_getInstanceVariable", new Class[] { long.class, byte[].class, long[].class });
			long[] resultPtr = new long[1];
			method.invoke(null, new Object[] { new Long(delegateId), name, resultPtr });
			return resultPtr;
		}
		else {
			method = clazz.getMethod("object_getInstanceVariable", new Class[] { int.class, byte[].class, int[].class });

...
import fr.minibilles.basics.ui.field.text.CompletionProposer.CompletionProposal;
...
public class CodeCompletionHandler {
	...
	private final CompletionProposer proposer;
	...
	public CodeCompletionHandler(CompletionProposer proposer) {
	...
	public void openRefreshCompletion(boolean forceOpen) {
		String string = styledText.getText();
		if (string.length() == 0) {
			completionShell.setVisible(false);
		} else {

			if ( forceOpen || completionShell.isVisible() ) {
				int index = styledText.getCaretOffset();
				String text = styledText.getText();
	
				int wordStart = index -1;
				while ( wordStart >= 0 && '\n' != string.charAt(wordStart) ) {
					wordStart--;
				}
				wordStart++;
				...
				CompletionProposal[] proposals = proposer.getProposals(text, index, word);

...
public final class SExpToModel {
	...
	/**
	 * Used to translates {@link SVariable} to effective model objects.
	 */
	...
	public void registerReference(Object receiver, String name, boolean many, SExp sexp) throws IOException {
		...
		if ( sexp.isVariable() ) {
			final String variableName = ((SVariable) sexp).getName();
			final Class<?> type = method.getParameterTypes()[0];
			referenced = resolver.resolve(variableName, type);
			// referenced can't be null here, if the variable is unknown an exception has been throw,
		} else {
			referenced = resolve(sexp.getValue());
		}
		...
		if ( referenced != null ) {
			...
			contextCurrent.registerPendingReference(sexp.getValue(), receiver, method);
		}
	}
